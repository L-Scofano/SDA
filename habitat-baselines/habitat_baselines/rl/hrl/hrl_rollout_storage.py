...
class HrlRolloutStorage(RolloutStorage):
    ...
    def insert(
        ...
        **kwargs,
        ...
        if aux_infos is not None and len(aux_infos) > 0:
            if aux_infos['dist_to_goal'].dtype == torch.float64:
                aux_infos['dist_to_goal'] = aux_infos['dist_to_goal'].to(
                    dtype=torch.float32)
            if aux_infos['rot_dist_to_goal'].dtype == torch.float64:
                aux_infos['rot_dist_to_goal'] = aux_infos['rot_dist_to_goal'].to(
                    dtype=torch.float32)
            self.buffers.set(
                (
                    self._cur_step_idxs[should_inserts] + 1,
                ...
                {'aux_infos': aux_infos[should_inserts]},
                strict=False,
            )
    ...
    def data_generator(self, advantages, num_batches, latent=None, history=None) -> Iterator[DictTree]:
        """
        ...
        for inds in torch.randperm(num_environments).chunk(num_batches):
            ...
            if latent is not None:
                if history is not None:
                    yield batch.to_tree(), latent[:, inds].reshape(-1, latent.shape[-1]), history[:, inds].reshape(-1, history.shape[-2], history.shape[-1])
                else:
                    yield batch.to_tree(), latent[:, inds].reshape(-1, latent.shape[-1])
            else:
                yield batch.to_tree()
    ...
    def insert_first_observations(self, batch, aux_infos):
        super().insert_first_observations(batch, aux_infos)
