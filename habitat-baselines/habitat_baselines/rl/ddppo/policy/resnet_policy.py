...
from habitat_baselines.rl.ddppo.policy.noise_sensor.redwoodnoise import RedwoodDepthNoiseModel
        ...
        **kwargs,
        ...
        super().__init__(
            PointNavResNetNet(
                ...
                rnn_type=rnn_type,
                phase=config.habitat_baselines.phase,
                out_dim_trajectory = config.habitat_baselines.out_dim_trajectory,
                backbone=backbone,
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if not self.is_blind:
            ...
            self.redwood_noise_model = RedwoodDepthNoiseModel()
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        for k in self.visual_keys:
            ...
            if k == 'spot_head_stereo_depth_sensor':
                obs_k = obs_k.squeeze()
                obs_k = obs_k.cpu().numpy()
                obs_k = self.redwood_noise_model.apply(obs_k)
                obs_k = torch.from_numpy(obs_k).unsqueeze(0).unsqueeze(1)
                # put on GPU
                obs_k = obs_k.to(observations[k].device)
                # plt.imshow(obs_k_noised)
                # plt.savefig('noisy_depth.png')
                # plt.imshow(obs_k)
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        self.out_dim_trajectory = 0 if self.phase == 'baseline' else out_dim_trajectory #128 #! TODO: put into config
        self.state_encoder = build_rnn_state_encoder(
            (0 if self.is_blind else self._hidden_size) + rnn_input_size + self.out_dim_trajectory,
            self._hidden_size,
            rnn_type=rnn_type,
    ...
    def forward(
        ...
        if not self.is_blind:
            ...
            visual_feats = self.visual_fc(visual_feats) #! [1,512] after Resnet and after FC
        ...
        else: #! Maps from 2 to 32 actions
            prev_actions = self.prev_action_embedding(
        ...
        out = torch.cat(x, dim=1) # we have encoded images from robot and previous actions encoded
