...
from habitat_baselines.trajectory.model import TrajectoryEncoder, AdaptationModule, TrajectoryTransformer
...
class NetPolicy(nn.Module, Policy):
    ...
    def __init__(
        self, net, action_space, policy_config=None, aux_loss_config=None, config=None,
    ):
        ...
        self.traj_encoder = config.habitat_baselines.traj_encoder
        ...
        # TODO-REFACTOR : fix this hardcoding
        if self.phase != 'baseline':  
            self.n_frames_to_consider = 20
            # NOTE: remember to change this based on the config!
            self.out_dim_trajectory = config.habitat_baselines.out_dim_trajectory
            # self.device = get_device(self.config)
            if self.traj_encoder == 'transformer':
                self.coordinates = 4 # TODO Add to config
                self.trajectory_encoder = TrajectoryTransformer(input_dim = self.coordinates, hidden_dim = self.coordinates, 
                                                                output_dim = self.out_dim_trajectory, time_dim = self.n_frames_to_consider)
                
                
            # TODO-REFACTOR : externalize hidden dimension in config
            elif self.traj_encoder == 'mlp':
                self.trajectory_encoder = TrajectoryEncoder(self.n_frames_to_consider*4, 128, self.out_dim_trajectory)

            # Stage 2: Adaptation Module
            # TODO-REFACTOR : externalize this 512 + 2 in config
            if self.phase == 'second':
                self.adaptation_module = AdaptationModule(512+2, self.n_frames_to_consider, 128, self.out_dim_trajectory)
    ...
    def act(
        ...
        if encode_history_and_return != None:
            if encode_history_and_return[0]=='phase1':
                if self.traj_encoder == 'transformer':
                    reshaped_history_input = encode_history_and_return[1].view(encode_history_and_return[1].shape[0], -1, self.coordinates)
                    mask = (reshaped_history_input == 0).all(dim=-1).to(reshaped_history_input.device)
                    z = self.trajectory_encoder(reshaped_history_input, mask)
                elif self.traj_encoder == 'mlp':
                    z = self.trajectory_encoder(encode_history_and_return[1])
                return z
            elif encode_history_and_return[0]=='phase2':
                z = self.adaptation_module(encode_history_and_return[1]).squeeze(1)
                features, rnn_hidden_states, _ = self.net(
                observations, rnn_hidden_states, prev_actions, masks, latent=z,
                ) 
                return z, features

        features, rnn_hidden_states, _ = self.net(
            observations, rnn_hidden_states, prev_actions, masks, latent=latent,
        ) # [n_envs,512], [n_envs,4,512]  ##### OBIETTIVO: IL NOSTRO Z #####
        distribution = self.action_distribution(features)
        value = self.critic(features) #! Linear layer [n_envs,512]->[n_envs]
        ...
        if deterministic:
            ...
            action = distribution.sample() #! [n_envs, 2]

        action_log_probs = distribution.log_probs(action) #! [n_envs]
    ...
    def get_value(self, observations, rnn_hidden_states, prev_actions, masks, latent=None):
    ...
    def evaluate_actions(
        ...
        action,
        rnn_build_seq_info: Dict[str, torch.Tensor],
        latent=None, # False friend -> it's the trajectory
        history=None,
        aux_infos=None
    ):  
        if latent is not None:
            if self.traj_encoder == 'transformer':
                latent_reshaped = latent.view(latent.shape[0], -1, self.coordinates)
                mask = (latent_reshaped == 0).all(dim=-1).to(latent_reshaped.device)
                latent = self.trajectory_encoder(latent_reshaped, mask)
            elif self.traj_encoder == 'mlp':
                latent = self.trajectory_encoder(latent)
        if history is not None:
            z = latent
            latent = self.adaptation_module(history).squeeze(1)
            z_hat = latent
