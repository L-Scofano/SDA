...
@baseline_registry.register_auxiliary_loss(name="cpcb")
class CPCB(CPCA):
    pass

@baseline_registry.register_auxiliary_loss(name="risk")
class Risk(ActionConditionedForwardModelingLoss):
    def __init__(
        self,
        action_space: gym.spaces.Box,
        net: Net,
        ...
        assert (
            not net.is_blind
        ), "Risk only works for networks with a visual encoder"
        hidden_size = net.output_size
    ...
    def forward(self, aux_loss_state, batch):
        ...
        device = future_preds.device
        targets = batch['aux_infos']['dist_to_goal'].to(device)
        #target_inds = target_inds.flatten()[future_inds]
        #target_valids = target_valids.flatten()[future_inds]

        targets = masked_index_select(
            targets, target_inds, target_valids
        )

        logits = self._predictor(
            future_preds
        )
        loss = F.mse_loss(logits, targets, reduction="none")
        loss = masked_mean(
            loss,
            target_valids.unsqueeze(-1),
        )
...
@baseline_registry.register_auxiliary_loss(name="compass")
class Compass(ActionConditionedForwardModelingLoss):
    def __init__(
        self,
        action_space: gym.spaces.Box,
        net: Net,
    ...
    def forward(self, aux_loss_state, batch):
        ...
        device = future_preds.device
        targets = batch['aux_infos']['rot_dist_to_goal'].to(device)
        #target_inds = target_inds.flatten()[future_inds]
        #target_valids = target_valids.flatten()[future_inds]
