...
from habitat_baselines.trajectory.model import TrajectoryEncoder
...
class PPOTrainer(BaseRLTrainer):
    ...
    def __init__(self, config=None):
        ...
        # NOTE: remember to change this based on the config!
        self.out_dim_trajectory = self.config.habitat_baselines.out_dim_trajectory
        #self.device = get_device(self.config)

        #self.insert_human_trajectory = True
        self.config = config
        self.phase = self.config.habitat_baselines.phase #'second' 
        

    def super_latent_reset(self):
        # TODO-REFACTOR : externalize 128 in config
        super_latent_ = torch.zeros(128, self.config.habitat_baselines.num_environments, self.out_dim_trajectory)
        return super_latent_
    
    def human_trajectory_reset(self):
        human_trajectory = torch.zeros(self.config.habitat_baselines.num_environments, self.n_frames_to_consider, 4)
        return human_trajectory
    
    def super_human_trajectory_reset(self):
        # TODO-REFACTOR : externalize 128 in config
        super_human_trajectory = torch.zeros(128, self.config.habitat_baselines.num_environments, self.n_frames_to_consider, 4)
        return super_human_trajectory

    def super_history_reset(self):
        # TODO-REFACTOR : externalize + 2 in config
        super_history = torch.zeros(128, self.config.habitat_baselines.num_environments, self.n_frames_to_consider, 512+2)
        return super_history
    
    def history_reset(self):
        # TODO-REFACTOR : externalize + 2 in config
        history = torch.zeros(self.config.habitat_baselines.num_environments, self.n_frames_to_consider, 512+2)
        return history
    ...
    def _init_train(self, resume_state=None):
        ...
        if resume_state is not None:
            print(f"\n\n\nResuming state {resume_state.keys()}\n\n\n")
            ...
            #print("resuming trajectory encoder")
            #self.trajectory_encoder.load_state_dict(resume_state["trajectory_encoder"])
            #print("loaded weights for trajectory encoder")
            # resuming the human trajectory
            #! self.human_trajectory = resume_state["human_trajectory"]
            #! self.super_latent_ = resume_state["super_latent"]
        ...
        self.device = get_device(self.config)
        #! self.trajectory_encoder = self.trajectory_encoder.to(self.device)
        ...
        self._agent = self._create_agent(resume_state) #! Initialized the agents
        if self._is_distributed:
        ...
        observations = self.envs.reset() # !This is where the observations are reset! Both for agent 0 (robot) and agent 1(humanoid)
        aux_infos = [{ 'dist_to_goal': torch.ones(1), 'rot_dist_to_goal': torch.zeros(1)} for _ in range(len(observations))]
        observations = self.envs.post_step(observations)
        aux_infos = batch_obs(aux_infos, device=self.device)
        batch = batch_obs(observations, device=self.device)
        batch = apply_obs_transforms_batch(batch, self.obs_transforms)  # type: ignore

        # Initialize latents history
        self.super_latent_ = self.super_latent_reset()
        self.human_trajectory = self.human_trajectory_reset()
        self.super_human_trajectory = self.super_human_trajectory_reset()
        self.super_history = self.super_history_reset()
        self.history = self.history_reset()
        ...
        self._agent.rollouts.insert_first_observations(batch, aux_infos)
        #! Ora non fa niente ma può essere un modo per lo storage degli eventi (trajectories and orientations)
        #! Important: self._agent.rollouts._active_storages[1].buffers['actions'].shape = 129,1,23
        #! while self._agent.rollouts._active_storages[0].buffers['actions'].shape = 129,1,2
        #! What do 23 and 2 represent?
    ...
    def save_checkpoint(
        ...
        checkpoint = {
            **self._agent.get_save_state(),
            ...
            #"super_latent": self.super_latent_,
            "config": self.config,
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        with g_timer.avg_time("trainer.sample_action"), inference_mode():
            ...
            ) #! we need step_batch['orientations'], step_batch['root']
            # ogni volta cambiano tutti i valori degli agenti
            
            # if phase 2
            # history_embed = resnet(self.history)
            # z_hat = self.adamodule(history_embed)
            # latent_ = z_hat
            ...
            if self.phase != 'baseline':
                ...
                current_human_localization = step_batch['observations']['agent_1_localization_sensor'].unsqueeze(1)
                self.human_trajectory = self.human_trajectory.to(current_human_localization.device) #! CHECK IF DISTRIBUTED NOT WORKING	
                self.human_trajectory = torch.cat((self.human_trajectory[:, 1:], current_human_localization), dim=1)
                

                # ENCODE INTO A LATENT SPACE
                human_trajectory_reshaped = self.human_trajectory.view(-1, self.n_frames_to_consider*4)
                z = self._agent.actor_critic.act(
                ...
                # Phase 1
                self.latent_ = z
                self.super_latent_ = self.super_latent_.to(current_human_localization.device)
                self.super_latent_ = torch.cat((self.super_latent_[1:], z.unsqueeze(0)), dim=0)
                self.super_human_trajectory = self.super_human_trajectory.to(current_human_localization.device)
                self.super_human_trajectory = torch.cat((self.super_human_trajectory[1:], self.human_trajectory.unsqueeze(0)), dim=0)
            ...
            if self.phase == 'second':
                self.history = self.history.to(current_human_localization.device)
                z_hat, features = self._agent.actor_critic.act(
                    step_batch["observations"],
                    step_batch["recurrent_hidden_states"],
                    step_batch["prev_actions"],
                    step_batch["masks"],
                ...
                xt_at = torch.cat([features, agent0_actions], dim=1).unsqueeze(1)
                self.history = torch.cat((self.history[:, 1:], xt_at), dim=1)
                self.super_history = self.super_history.to(current_human_localization.device)
                self.super_history = torch.cat((self.super_history[1:], self.history.unsqueeze(0)), dim=0)
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        with g_timer.avg_time("trainer.step_env"):
            ...
            ] #! qui dentro passa per il kinematic
        ...
        def get_risk_value(dist_to_goal):
            sigma = 0.375
            return np.float32(-np.exp(
                -(dist_to_goal - 1.5) ** 2 / (2 * sigma ** 2)) + 1.).item()

        for info in infos:
            val = get_risk_value(info['dist_to_goal'])
            angle = info['rot_dist_to_goal'] / 3.14
            aux_infos.append(
                {'dist_to_goal': [val], 'rot_dist_to_goal': [angle]})
            
        aux_infos = batch_obs(aux_infos, device=self.device)

        # Reset trajectory and history if episode ends
        with inference_mode():
            for idx, dn in enumerate(dones):
                if dn==True:
                    # Initialize latents history
                    #self.super_latent_[:,idx] = 0.
                    self.human_trajectory[idx] = 0.
                    self.history[idx] = 0.
    ...
    def _update_agent(self):
        with inference_mode():
            ...
            next_value = self._agent.actor_critic.get_value(
                ...
                step_batch["prev_actions"],
                step_batch["masks"],
                self.latent_, # ! Check if this is correct or needs to be stored in the rollouts
                **step_batch_lens,
            ) # next_value is the value of the last states for both agents, [n_envs,1,2]
        ...
        if self.phase == 'first':
            losses = self._agent.updater.update(self._agent.rollouts, 
                                                latent=self.super_human_trajectory.clone().reshape(-1,  self.super_human_trajectory.shape[1], self.n_frames_to_consider*4))
        elif self.phase == 'second':
            # TODO-REFACTOR : externalize + 2 in config
            losses = self._agent.updater.update(self._agent.rollouts, 
                                                latent=self.super_human_trajectory.clone().reshape(-1,  self.super_human_trajectory.shape[1], self.n_frames_to_consider*4),
                                                history=self.super_history.clone().reshape(-1, self.super_human_trajectory.shape[1], self.n_frames_to_consider, 512+2))
        else:
            losses = self._agent.updater.update(self._agent.rollouts)
    ...
    def train(self) -> None:
        ...
        resume_run_id = None
        if resume_state is not None and self.phase!='second':
            self._agent.load_state_dict(resume_state)
            # print("Loading the trajectory encoder")
            # self.trajectory_encoder.load_state_dict(resume_state["trajectory_encoder"]) 
        ...
        if self.phase == 'second':
            ...
            stage_1_ckpt = '/media/hdd/luca_s/code/SocialNav3.0/checkpoints_social_nav/stage1/ckpt_stage1_d128_t20.pth'
            stage_1_ckpt_dict = self.load_checkpoint(stage_1_ckpt, map_location="cpu")
            self._agent.load_state_dict(stage_1_ckpt_dict)
    ...
    def _eval_checkpoint(
        ...
        #if self.config.habitat_baselines.eval.should_load_ckpt:
        #    print("\n\n\n\nLoading the trajectory encoder\n\n\n\n")
        #    self.trajectory_encoder.load_state_dict(ckpt_dict["trajectory_encoder"])
