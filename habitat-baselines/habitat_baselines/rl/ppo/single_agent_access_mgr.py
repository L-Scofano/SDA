...
class SingleAgentAccessMgr(AgentAccessMgr):
    ...
    def _create_policy(self) -> NetPolicy:
        ...
        phase=self._config.habitat_baselines.phase
        # Set actor_critic.adaptation_module with required_grad=True, all other parameters with required_grad=False
        if phase == "second":
            for name, param in actor_critic.named_parameters():
                if name.startswith("adaptation_module"):
    ...
    def load_state_dict(self, state: Dict) -> None:
        #! Added
        filtered_dict = {k: v for k, v in state["state_dict"].items() if k in self._actor_critic.state_dict()}

        for k, v in state["state_dict"].items():
            self.actor_critic.state_dict()[k].copy_(filtered_dict[k])
        #! self._actor_critic.load_state_dict(filtered_dict)
