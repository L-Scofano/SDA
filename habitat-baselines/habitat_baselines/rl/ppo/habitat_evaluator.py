...
def super_human_trajectory_reset(num_environments, n_frames_to_consider):
    super_human_trajectory = torch.zeros(128, num_environments, n_frames_to_consider, 4)
...
def super_history_reset(num_environments, n_frames_to_consider):
    ...
    super_history = torch.zeros(128, num_environments, n_frames_to_consider, 512+2)
...
class HabitatEvaluator(Evaluator):
    ...
    def evaluate_agent(
        ...
        self.super_human_trajectory = super_human_trajectory_reset(n_environs, self.n_frames_to_consider)
        self.super_history = super_history_reset(n_environs, self.n_frames_to_consider)
        self.human_trajectory = human_trajectory_reset(n_environs, self.n_frames_to_consider)
        ...
        while (
            ...
            with inference_mode():
                ...
                if self.phase == 'first':
                    ...
                    current_human_localization = batch['agent_1_localization_sensor'].unsqueeze(1)
                    self.human_trajectory = self.human_trajectory.to(current_human_localization.device)
                    ...
                    self.history = self.history.to(batch['agent_1_localization_sensor'].device)
