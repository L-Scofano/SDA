...
class PPO(nn.Module, Updater):
    ...
    def update(
        self,
        rollouts: RolloutStorage, latent=None, history=None,
    ) -> Dict[str, float]:
        ...
        for epoch in range(self.ppo_epoch):
            ...
            data_generator = rollouts.data_generator(
                advantages, self.num_mini_batch, latent=latent, history=history
            )
            ...
            for _bid, batch in enumerate(data_generator):
                if latent is not None:
                    batch_latent = batch[1]
                    batch_history = None
                    if history is not None:
                        batch_history = batch[2]
                    batch = batch[0]
                self._update_from_batch(
                    batch, epoch, rollouts, learner_metrics, latent=batch_latent, history=batch_history
                )
