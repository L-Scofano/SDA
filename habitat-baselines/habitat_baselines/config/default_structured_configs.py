...
@dataclass
class CPCBLossConfig(AuxLossConfig):
    """Action-conditional contrastive predictive coding loss"""
...
@dataclass
class RiskLossConfig(AuxLossConfig):
    """Action-conditional risk predictive loss"""
...
@dataclass
class CompassLossConfig(AuxLossConfig):
    """Action-conditional compass predictive loss"""
...
class HabitatBaselinesConfig(HabitatBaselinesBaseConfig):
    ...
    phase: str = "baseline"
    out_dim_trajectory: int = 128
    traj_encoder: str = "transformer"
    n_frames_to_consider: int = 20
...
cs.store(
    package="habitat_baselines.rl.auxiliary_losses.cpcb",
    group="habitat_baselines/rl/auxiliary_losses",
    name="cpcb",
    node=CPCBLossConfig,
)
...
cs.store(
    package="habitat_baselines.rl.auxiliary_losses.risk",
    group="habitat_baselines/rl/auxiliary_losses",
    name="risk",
    node=RiskLossConfig,
)
...
cs.store(
    package="habitat_baselines.rl.auxiliary_losses.compass",
    group="habitat_baselines/rl/auxiliary_losses",
    name="compass",
    node=CompassLossConfig,
)
